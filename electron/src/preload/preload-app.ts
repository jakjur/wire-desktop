...
const logger = {
  error: (...logs: string[]) => ipcRenderer.invoke('LOG', 'error', path.basename(__filename), logs),
  info: (...logs: string[]) => ipcRenderer.invoke('LOG', 'info', path.basename(__filename), logs),
  log: (...logs: string[]) => ipcRenderer.invoke('LOG', 'log', path.basename(__filename), logs),
  warn: (...logs: string[]) => ipcRenderer.invoke('LOG', 'warn', path.basename(__filename), logs),
};

const locale = {
  getCurrent(): Promise<SupportedI18nLanguage> {
    return ipcRenderer.invoke('LOCALE', 'getCurrent');
  },
  getLANGUAGES(): Promise<SupportedI18nLanguageObject> {
    return ipcRenderer.invoke('LOCALE', 'getLanguages');
  },
};

const EnvironmentUtil = {
  getPlatform: (): Promise<typeof platform> => ipcRenderer.invoke('ENVIRONMENTUTIL', 'getPlatform'),
};
